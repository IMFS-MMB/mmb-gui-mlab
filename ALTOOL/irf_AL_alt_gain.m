% correct for mu
function IRF = irf_AL_alt_gain(dr,imp,smpl,AL_,gain)
...
exits = 0;
[~,exit_var_loc] = ismember({'inflation','interest','output','outputgap'},tmp);
...
while t<smpl
    ...
    if t == 1
        tmp_vars = mu + T * Y_init + R * imp;
    else
        tmp_vars = mu + T * tmp_vars;
    end
    ...
    % steady state (zero)
    %     exit = any(abs(filteratt(exit_var_loc,t)) > 4 * tmp);
    %     if exit
