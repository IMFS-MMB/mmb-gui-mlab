...
// **************************************************************************************
// ********** FLEXIBLE PRICE, WAGE DEPOSIT/LOAN RATES ECONOMY ***************************
// **************************************************************************************
...
exp(eps_l)*exp(l_pf)^(phi) = exp(lam_pf)*exp(w_pf)*(exp(eps_l)-1); // Unions, labor supply, no adjustment costs (4)

exp(c_pf) + exp(q_hf) * ( exp(h_pf) - exp(h_pf(-1)) ) + exp(d_pf)  = exp(w_pf) * exp(l_pf)
   + (1+exp(r_df(-1)))*exp(d_pf(-1)) + exp(J_Rf)/gamma_p ;  // patient household budget constraint (6)


////***********   2) IMPATIENT HHs ********************************************************7

  (1-a_i)*exp(ee_z)*(exp(c_if) - a_i*exp(c_if(-1)))^(-1)  = exp(lam_if); // CON rescaling (7)

j * (exp(ee_j))  / exp(h_if) - exp(lam_if) * exp(q_hf) + beta_i * exp(lam_if(+1)) * exp(q_hf(+1))  + exp(s_if) * exp(m_i) *exp(q_hf(+1))   = 0;    // (8) s_if is multiplier on borrowing constraint
exp(lam_if) - beta_i * exp(lam_if(+1)) * (1+exp(r_bhf)) = exp(s_if) * (1+exp(r_bhf)); // (9)
...
exp(w_pf) =    ni  * (1-alpha) * exp(y_ef) / ( exp(l_pdf) * exp(xf) ); // (18) FOC labor patient households
exp(w_if) = (1-ni) * (1-alpha) * exp(y_ef) / ( exp(l_idf) * exp(xf) ); // (19) FOC labor impatient households
...
(1+exp(r_bef)) * exp(b_eef) = exp(m_e) * exp(q_kf(+1)) * exp(k_ef) * (1-deltak); // borrowing constraint entrepreneurs (24)
...
exp(J_Rf)  = exp(Yf)*(1 - (1/exp(xf))) ; // (32) aggregate retail profits

// (33) FOC price-setting of retailers, case without adjustment costs
exp(xf)*(exp(eps_y)-1) = exp(eps_y);

////************  7) AGGREGATION & EQUILIBRIUM  ************************************************

exp(Cf)              = gamma_p * exp(c_pf) + gamma_i * exp(c_if) + gamma_e * exp(c_ef);
