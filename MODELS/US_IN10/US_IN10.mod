...
data_CCf data_IHf data_IKf zata_GDPf
//**************************************************************************
// Modelbase Variables                                                   //*
...
exp(bf) - exp(rf(-1)+bf(-1)-TRENDY) ;

//% 9
exp(qf+uc1f) = exp(a_z+a_j-h1f)*JEI +
BETA1*exp(TRENDY)*(1-DH)*exp(qf(+1)+TRENDQ+uc1f(+1)-TRENDY) +
M*exp(lmf+(qf(+1)+ TRENDQ -rf)) ;

//% 10
bf = log(M) + (qf(+1)+TRENDQ) + h1f  - rf   ;

//% 11
exp(uc1f) = BETA1*exp(TRENDY)*exp(rf+uc1f(+1)-TRENDY) + exp(lmf) ;
...
(exp(IK_SS)/(exp(CC_SS)+exp(QQ_SS+IH_SS)+exp(IK_SS)))*(data_IKf-TRENDK) +
(exp(QQ_SS+IH_SS)/(exp(CC_SS)+exp(QQ_SS+IH_SS)+exp(IK_SS)))*(data_IHf-TRENDH) ;
...
//stoch_simul(irf=20) data_CC data_IK data_IH data_QQ zata_GDP data_RR ;
