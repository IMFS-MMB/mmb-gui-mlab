%**********************************************************************************************************************%
%                    This is the main file of the Macroeconomic Model Data Base (MMB).                                 %
%**********************************************************************************************************************%
%
...
global modelsvec;                         % List of model(s) chosen for an exercise: modelsvec(model_number)==1 if the model number model_number is chosen (see variable names below for model_number) 

global rulenames;                         % List of names for the monetary policy rules
global rulenamesshort;                    % List of acronyms for the monetary policy rules
global rulenamesshort1;                   % List of acronyms for the monetary policy rules
global common_rule;                       % The set of all coefficients of a policy rule 
global model_with_rule;                   % List of numbers of models with a model-specific rule   
global model_without_rule;                % List of numbers of models without a model-specific rule
global myrulecolor;                       % The color of each policy rule
global Number_rule;                       % Number of common monetary policy rules used        
global rule;                              % List of rule(s) chosen for an exercise: rule(i)==1 if the i-th rule is chosen and 0 otherwise
global data;                              % A group of coefficients of the user-specified rule in the second exercise (MMBOPT2)
global shocks; shocks=zeros(2,1);         % Common Shocks (a Monetary Policy shock and a Fiscal Policy shock)

global option1;option1=0;                 % Autocorrelation Functions (ACFs) will be plotted if option1==1
global option2;option2=0;                 % Impulse Response Functions (IRFs) will be plotted if option2==1
global option3; option3=0;                % Compute the IRFs to a contemporaneous combination of shocks in the MMBOPT2. 
global option4; option4=1;                % Plot selected (0) or all variables (1) in the MMBOPT2.
global option5;option5=0;                 % Show the unconditional variance in the Matlab console (1)
global option6;option6=0;                 % Choose model specific shocks (1) in the MMBOPT2. 
global results;results=num2str('results.xls'); % The name of output file specified by the user 
global horizon; horizon = 20;             % Default number of periods to be plotted
global NK_Models;
...
global AL_Models;
global AL_Info;
global OSenvironment;				       % Variable to control if Matlab is run on Windows or in a Unix environment
%%
...
mycolor = char('r','g','b','c','m','y','k',':r',':g',':b',':c',':m',':y',':k',...
                       ...
                       '-.r','-.g','-.b','-.c','-.m','-.y','-.k',...
                       '+-r','+-g','+-b','+-c','+-m','+-y','+-k',...
                       'o-r','o-g','o-b','o-c','o-m','o-y','o-k',...
                       'x-r','x-g','x-b','x-c','x-m','x-y','x-k',...
                       '*-r','*-g','*-b','*-c','*-m','*-y','*-k',...
                       '^-r','^-g','^-b','^-c','^-m','^-y','^-k',...
                       '<-r','<-g','<-b','<-c','<-m','<-y','<-k',...
                       '>-r','>-g','>-b','>-c','>-m','>-y','>-k',...
...
%%% PATHS TO WRITE EXCEL FILES
javaaddpath([currentpath num2str('/MMB_OPTIONS/xlwrite/poi_library/poi-3.8-20120326.jar')]);
javaaddpath([currentpath num2str('/MMB_OPTIONS/xlwrite/poi_library/poi-ooxml-3.8-20120326.jar')]);
javaaddpath([currentpath num2str('/MMB_OPTIONS/xlwrite/poi_library/poi-ooxml-schemas-3.8-20120326.jar')]);
javaaddpath([currentpath num2str('/MMB_OPTIONS/xlwrite/poi_library/xmlbeans-2.3.0.jar')]);
javaaddpath([currentpath num2str('/MMB_OPTIONS/xlwrite/poi_library/dom4j-1.6.1.jar')]);
